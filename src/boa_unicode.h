...
boa_forceinline int boa_convert_utf16_to_utf8_replace_default(boa_buf *buf, const uint16_t **ptr, const uint16_t *end) {
	return boa_convert_utf16_to_utf8_replace(buf, ptr, end, boa_utf8_replacement_character, 3);
}
boa_forceinline int boa_convert_utf8_to_utf16_replace_default(boa_buf *buf, const char **ptr, const char *end) {
	return boa_convert_utf8_to_utf16_replace(buf, ptr, end, boa_utf16_replacement_character, 1);
}

boa_forceinline int boa_convert_utf16_to_utf8_replace_empty(boa_buf *buf, const uint16_t **ptr, const uint16_t *end) {
	return boa_convert_utf16_to_utf8_replace(buf, ptr, end, NULL, 0);
}
boa_forceinline int boa_convert_utf8_to_utf16_replace_empty(boa_buf *buf, const char **ptr, const char *end) {
